        ...
        try{
            if(obj[key] instanceof Function){
                function warppedAction() {
                    let err;
                    let out;
                    funcStack.push(name+"."+mapFunc(key));
                    let currentName=funcStack.join("->");
                    const start = Game.cpu.getUsed();
                    try {// 保证报错了能正确统计
                        out = obj["$_$"+key].apply(this, arguments);
                    }catch (e) {
                        err=e;// 将错误带出来
                    }finally {
                        const end =  Game.cpu.getUsed();
...
<script type="text/javascript" src="https://screeps.com/a/vendor/lodash/lodash.js"></script>
...
let head=
    ...
    +"<tr><td>"
    +"统计(AVG) "+tickTimes+" tick 的数据 "+'<span onclick="copyText()">[复制dom到剪切板]</span>'+"</td><td>"
    +"total</td><td>"
    +"hardLs</td><td>"
    +"pure</td><td>"
    +"tickTimes</td><td>"
...
function getFuncTree(currentTick,tree,name,head="",deep=0){
    ...
    for(let n of keys){
        ...
        if(tree[n])h+=('<span onclick="change('+"'"+fullName+"'"+')">['+(closeMap[fullName]?"+":"-")+']</span>');
        else h+="[_]";
        ...
        if(tree[n]&&!closeMap[fullName])out+=getFuncTree(currentTick,tree[n],fullName,head+(name?(last==n?"   ":" | "):""),deep+1);
    }
    return out
}

update=()=>{document.getElementById("${Game.time}").innerHTML=head+getFuncTree(current)+tail};
update();
...
let pro={
    saveSum:1500, // 保存的帧数 ！！！不能在运行时修改
    skipTick:10, // 保存的帧数最小单位（n tick存一次）  ！！！不能在运行时修改
