...
export function check(creep: Creep) {
  ...
  if (hostile.length > 0 && !creep.room.memory.hasHostile) {
    creep.room.memory.hasHostile = true;
  }
  if (hostile.length == 0) {
    creep.room.memory.hasHostile = false;
  }
}
...
export class remoteMiner extends creepExt {
  ...
  work(): void {
    ...
    if (source && this.pos.roomName == source.pos.roomName) {
      if ((container && container.store.getFreeCapacity() > 0) || !container) {
        const result = this.harvest(source.pos.findInRange(FIND_SOURCES, 1)[0]);
