        ...
        for (const room in Game.rooms) {
            try {
                if (Game.rooms[room].controller.my) {
                    ...
                    this.text(`等级:${level}`, { x: 13, y: i + 4 });
                } else if (
                    Game.rooms[room].controller.reservation.username ==
                    'EricGuo'
                ) {
                    i++;
                    const reservationProgress =
                        Game.rooms[room].controller.reservation.ticksToEnd;
                    const reservationTotal = 5000;
                    this.text(`${Game.rooms[room].name}:`, { x: 1, y: i + 4 });
                    this.barGraph([reservationProgress, reservationTotal], {
                        x: 4.5,
                        y: i + 4,
                    });
                }
